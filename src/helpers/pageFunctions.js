...
export function createCityElement(cityInfo) {
  ...
  const buttonWeather = createElement('button', 'forecast-button');
  buttonWeather.innerText = 'Ver previsão';
  buttonWeather.addEventListener('click', async () => {
    try {
      ...
      const weekDaysWeather = data.forecast.forecastday;
      const allDays = await Promise.all(
        weekDaysWeather.map((days) => ({
          date: days.date,
      ...
      showForecast(allDays);
    } catch (error) {
      console.error(error);
    }
  });
  cityElement.appendChild(buttonWeather);
...
export async function handleSearch(event) {
  ...
  const allCitiesWeather = await Promise.all(cities
    .map((cityElement) => getWeatherByCity(cityElement.url)));
  const city = await Promise.all(allCitiesWeather
    .map((cityElement) => createCityElement(cityElement)));
  const ulCities = document.getElementById('cities');
  city.map((element) => ulCities.appendChild(element));
}
